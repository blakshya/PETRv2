...
class LoadMultiViewImagesWithPrevFrames(object):
    """Load multi-view images from current and previous frames.
    
    This pipeline extends the original loading functionality to handle multiple
    consecutive frames for temporal modeling.
    
    Args:
        to_float32 (bool): Whether to convert images to float32. Default: True
        color_type (str): Color type of images. Default: 'color'
        file_client_args (dict): Arguments for file client. Default: dict(backend='disk')
    """
    
    def __init__(self, 
                to_float32=True,  # Changed to True to match original pipeline
                file_client_args=dict(backend='disk'),
                color_type='color'):  # Changed to 'color' to match original
    ...
    def __call__(self, results):
        """Load current frame and previous frames images.
        
        Args:
            results (dict): Result dict containing image filepaths.
            
        Returns:
            dict: Updated dict with loaded images and combined transformations.
        """
        ...
        for name in filename:
            ...
            if self.to_float32:
                img_data = img_data.astype(np.float32)  # Convert to float32
            img.append(img_data)
        ...
        if 'prev_frames' in results:
            prev_imgs = []
            for frame in results['prev_frames']:
                frame_imgs = []
                for name in frame['img_filename']:
                    img_data = mmcv.imread(name, self.color_type)
                    if self.to_float32:
                        img_data = img_data.astype(np.float32)  # Convert to float32
                    frame_imgs.append(img_data)
                prev_imgs.append(frame_imgs)
                
                # Add transformation matrices
                results['lidar2img'].extend(frame['lidar2img'])
                results['intrinsics'].extend(frame['intrinsics'])
                results['extrinsics'].extend(frame['extrinsics'])
            
            results['prev_imgs'] = prev_imgs
        
        return results
    
@PIPELINES.register_module()
class LoadMultiViewImageFromMultiSweepsFilesWithPrevFrames(object):
    ...
    def __call__(self, results):
        ...
        # Process previous frames if available
        if 'prev_frames' in results:
            for frame in results['prev_frames']:
                frame_imgs = [mmcv.imread(p, self.color_type) for p in frame['img_filename']]
                if self.to_float32:
                    frame_imgs = [img.astype(np.float32) for img in frame_imgs]  # Convert to float32
                sweep_imgs_list.extend(frame_imgs)
                prev_ts = [lidar_timestamp - frame['timestamp']] * len(frame['img_filename'])
                timestamp_imgs_list.extend(prev_ts)
                
                # Add transformations
                for i in range(len(frame['img_filename'])):
                    if 'filename' in results:
                        results['filename'].append(frame['img_filename'][i])
                    results['lidar2img'].append(frame['lidar2img'][i])
                    results['intrinsics'].append(frame['intrinsics'][i])
                    results['extrinsics'].append(frame['extrinsics'][i])
